...
@section("content")
    <form class="form-horizontal" action="" method="post"  enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-4">
                <input type="text" class="form-control" id="inputEmail3" placeholder="请输入商铺地址" name="dizhi" value="{{old('dizhi',$shopcontents->dizhi)}}">
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-4">
                <select name="shoptype_id">
                    @foreach($shoptypes as $shoptype)
                        <option value="{{$shoptype->id}}" {{$shoptype->id==$shopcontents->shoptype_id?"selected":""}}>{{$shoptype->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                ...
                <input type="radio" name="zhun" value="1" {{$shopcontents->zhun==1?"checked":""}}>不准点
            </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <input type="radio" name="status" value="0" {{$shopcontents->zhun==0?"checked":""}}>以通过
                <input type="radio" name="status" value="1" {{$shopcontents->zhun==1?"checked":""}}>待审核
                <input type="radio" name="status" value="2" {{$shopcontents->zhun==2?"checked":""}}>未通过
            </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            @if($shopcontents->logo)
                <img src="/uploads/{{$shopcontents->logo}}" width="120" height="70">
